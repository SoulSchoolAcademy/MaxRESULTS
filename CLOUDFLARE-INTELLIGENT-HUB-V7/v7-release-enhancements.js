/* V7 RELEASE ENHANCEMENTS — functional Settings + Evidence proof center + visible V7 operating shell. */
(function(){
  'use strict';
  ...
  async function boot(){
    try{
      ...
      renderSettings();renderEvidence();markShellLive();
    }catch(e){
  ...
  function renderSettings(error){
    ...
    p.innerHTML=shell('Settings','OPERATING CONTROLS','<div class="v7settingsHero"><div><span class="v7liveDot"></span><b>V7 OPERATING CONTROLS</b><p>These are live application controls, not decorative settings. Changes are persisted to your V7 profile when the authenticated backend is available.</p></div><strong>'+Object.values(settings).filter(v=>v===true).length+' ACTIVE</strong></div><div class="v7settingsGrid">'+rows.map(([key,label,desc])=>'<article class="v7setting '+(settings[key]?'on':'off')+'"><div><b>'+label+'</b><p>'+desc+'</p><span class="v7settingState">'+(settings[key]?'ON · PERSISTED':'OFF · PERSISTED')+'</span></div><button class="btn '+(settings[key]?'purple':'')+'" data-setting="'+key+'">'+(settings[key]?'TURN OFF':'TURN ON')+'</button></article>').join('')+'</div><div class="v7card" style="margin-top:18px"><h2>Privacy law</h2><p class="v7muted">Private by default. Shared by choice. Collective by consent. Public by decision.</p><p class="v7muted">'+esc(error||'Settings are synchronized with the authenticated V7 profile when the backend is available. Browser persistence remains as a recovery layer.')+'</p></div>');
  ...
  function renderEvidence(error){
    ...
    const cards=notes.map(n=>{
      const tx=n.transaction||n;
      const status=tx.status||'unknown';
      const stages=[['Human Note',!!tx.human_note],['Naya Note',!!tx.naya_note],['Machine Note',!!tx.machine_note],['Intelligent Feed',!!tx.intelligent_feed],['Intelligent Block',!!tx.intelligent_block],['Receipt',!!tx.id]];
      const chain=stages.map(([label,ok])=>'<div class="'+(ok?'verified':'blocked')+'"><i>'+(ok?'✓':'!')+'</i><span>'+label+'</span></div>').join('');
      const failure=status==='failed'?'<div class="v7failure"><b>FAILURE / RECOVERY</b><span>Blocked at '+esc(tx.failure_stage||'unknown')+': '+esc(tx.failure_message||'unknown')+'</span><em>State preserved. Configure the blocked dependency and retry the same event.</em></div>':'<div class="v7proof">PERSISTENCE PROOF · Supabase transaction '+esc(tx.id||'—')+' observed in the authenticated ledger.</div>';
      return '<article class="v7evidenceCard"><div class="v7evidenceTop"><div><b>'+esc(n.type||'SMART NOTE')+'</b><span>'+esc(new Date(n.createdAt||tx.created_at).toLocaleString())+'</span></div><strong class="'+(status==='completed'?'good':'')+'">'+esc(status.toUpperCase())+'</strong></div><div class="v7chain">'+chain+'</div><div class="v7meta"><span><b>EVENT ID</b>'+esc(tx.id||'—')+'</span><span><b>IDEMPOTENCY</b>'+esc(tx.idempotency_key||'—')+'</span><span><b>SOURCE</b>'+esc(tx.human_note?.source||'human_input')+'</span><span><b>TIMESTAMP</b>'+esc(tx.created_at||n.createdAt||'—')+'</span></div>'+failure+'</article>';
    }).join('');
    const empty=notes.length?'':'<div class="v7card"><h2>No server events yet.</h2><p class="v7muted">The proof center stays empty rather than fabricating evidence. Create a real Smart Note and this ledger will populate from authenticated server state.</p></div>';
    const errorCard=error?'<div class="v7card" style="margin-top:18px"><h2>Live check blocked</h2><p class="v7muted">'+esc(error)+'</p></div>':'';
    p.innerHTML=shell('Evidence','PROOF CENTER','<div class="v7evidenceHero"><div><span>RELEASE INVARIANT</span><h2>Receipts, not claims.</h2><p>Each consequential Smart Note is reviewed as a linked chain from Human Note through Naya Note, Machine Note, Intelligent Feed, Intelligent Block and Evidence.</p></div><div class="v7evidenceCount"><b>'+notes.length+'</b><span>SERVER EVENTS</span></div></div><div class="v7evidenceLedger">'+cards+empty+'</div><div class="v7card" style="margin-top:18px"><h2>Release rule</h2><p class="v7muted">A receipt is not enough by itself. Ship status requires the authenticated event to survive refresh/re-entry and be independently observed at the exact public runtime.</p></div>'+errorCard);
  }
  function styles(){if(document.getElementById('v7ReleaseStyle'))return;const s=document.createElement('style');s.id='v7ReleaseStyle';s.textContent='.v7settingsHero{display:flex;justify-content:space-between;align-items:center;gap:20px;margin-bottom:16px;padding:18px 20px;border:1px solid #55e39a38;border-radius:18px;background:linear-gradient(135deg,#0d1d16,#0a0c10);box-shadow:inset 0 1px #fff4,0 15px 35px #0008}.v7settingsHero b{font-size:10px;letter-spacing:.12em}.v7settingsHero p{margin:6px 0 0;color:#d2cdd8;font-size:11px;line-height:1.5}.v7settingsHero>strong{font-size:11px;color:#55e39a;letter-spacing:.1em;white-space:nowrap}.v7liveDot{display:inline-block;width:8px;height:8px;border-radius:50%;background:#55e39a;box-shadow:0 0 15px #55e39a;margin-right:8px}.v7settingsGrid{display:grid;gap:14px}.v7setting{display:flex;align-items:center;justify-content:space-between;gap:20px;padding:22px;border:1px solid #ffffff20;border-radius:22px;background:linear-gradient(145deg,#14141b,#09090d);box-shadow:inset 0 1px #fff4,0 18px 42px #0009}.v7setting.on{border-color:#b27cff45}.v7setting b{font-size:15px}.v7setting p{max-width:820px;color:#d1cdd7;font-size:12px;line-height:1.55;margin:7px 0}.v7settingState{font-size:9px;font-weight:950;letter-spacing:.13em;color:#55e39a}.v7setting.off .v7settingState{color:#b8b4bf}.v7evidenceHero{display:flex;justify-content:space-between;gap:24px;padding:28px;border:1px solid #6675ff45;border-radius:26px;background:linear-gradient(135deg,#101a29,#0d0d15);box-shadow:inset 0 1px #fff4,0 25px 60px #000b}.v7evidenceHero span{font-size:9px;font-weight:950;letter-spacing:.15em;color:#9bbcff}.v7evidenceHero h2{font-size:31px;margin:8px 0}.v7evidenceHero p{max-width:820px;color:#ddd9e3;line-height:1.55;font-size:13px}.v7evidenceCount{min-width:150px;display:grid;place-items:center;border:1px solid #55b9ee35;border-radius:20px;background:#071018}.v7evidenceCount b{font-size:42px}.v7evidenceCount span{font-size:9px;letter-spacing:.13em}.v7evidenceLedger{display:grid;gap:14px;margin-top:18px}.v7evidenceCard{padding:22px;border:1px solid #ffffff20;border-radius:22px;background:#0b0b10;box-shadow:inset 0 1px #fff3,0 18px 45px #0009}.v7evidenceTop{display:flex;justify-content:space-between;gap:12px}.v7evidenceTop b{display:block;font-size:12px}.v7evidenceTop span{display:block;color:#aaa6b2;font-size:10px;margin-top:5px}.v7evidenceTop strong{font-size:9px;letter-spacing:.12em}.v7evidenceTop strong.good{color:#55e39a}.v7chain{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:8px;margin:18px 0}.v7chain div{min-height:72px;padding:10px;border:1px solid #ffffff15;border-radius:14px;display:flex;flex-direction:column;justify-content:space-between;background:#08080c}.v7chain div.verified{border-color:#55e39a35}.v7chain i{font-style:normal;font-weight:1000}.v7chain .verified i{color:#55e39a}.v7chain .blocked i{color:#e0b84a}.v7chain span{font-size:9px;line-height:1.25}.v7meta{display:grid;grid-template-columns:repeat(4,1fr);gap:8px}.v7meta span{padding:10px;border:1px solid #ffffff12;border-radius:12px;color:#d1cdd7;font-size:9px;word-break:break-word}.v7meta b{display:block;color:#8f8a97;font-size:8px;letter-spacing:.1em;margin-bottom:5px}.v7proof,.v7failure{margin-top:12px;padding:12px;border-radius:12px;border:1px solid #55e39a2b;background:#07130d;font-size:10px}.v7failure{border-color:#e0b84a35;background:#161207}.v7failure b,.v7failure span,.v7failure em{display:block}.v7failure em{margin-top:5px;color:#c9c1ad;font-style:normal}.v7muted{color:#d1cdd7;font-size:12px;line-height:1.55}.v7shellLive{display:inline-flex;align-items:center;gap:8px;margin-left:12px;padding:7px 10px;border:1px solid #55e39a45;border-radius:999px;background:#07130d;color:#bfffd8;font-size:9px;font-weight:950;letter-spacing:.12em}.v7shellLive i{width:7px;height:7px;border-radius:50%;background:#55e39a;box-shadow:0 0 12px #55e39a}@media(max-width:800px){.v7setting{align-items:flex-start;flex-direction:column}.v7settingsHero{align-items:flex-start;flex-direction:column}.v7evidenceHero{flex-direction:column}.v7chain{grid-template-columns:repeat(2,1fr)}.v7meta{grid-template-columns:1fr 1fr}}';document.head.appendChild(s)}
  function markShellLive(mode='V7 LIVE'){const brand=document.querySelector('.brand');if(brand&&!document.getElementById('v7ShellLive')){const b=document.createElement('span');b.id='v7ShellLive';b.className='v7shellLive';b.innerHTML='<i></i>'+mode;brand.appendChild(b)}const nav=document.querySelector('.nav');if(nav&&!document.getElementById('v7NavPulse')){const items=[...nav.querySelectorAll('button')];items.forEach(b=>{if(b.dataset.page==='notes'){b.style.borderColor='#55e39a45';b.style.background='linear-gradient(90deg,#10251b,#0a0b0d)';b.innerHTML='<span class="ico">▤</span><span>Smart Notes</span><span class="v7navBadge">LIVE</span>'}if(b.dataset.page==='settings'){b.style.borderColor='#b27cff38';b.innerHTML='<span class="ico">⚙</span><span>Settings</span><span class="v7navBadge">5</span>'}if(b.dataset.page==='evidence'){b.style.borderColor='#6675ff38';b.innerHTML='<span class="ico">◇</span><span>Evidence</span>'}});const marker=document.createElement('span');marker.id='v7NavPulse';nav.appendChild(marker)}}
  function integrate(){styles();renderSettings();renderEvidence();markShellLive();document.addEventListener('click',e=>{const b=e.target.closest('[data-page]');if(b&&b.dataset.page==='settings')setTimeout(renderSettings,0);if(b&&b.dataset.page==='evidence')setTimeout(renderEvidence,0)});const original=window.renderEvidence;window.renderEvidence=function(){if(typeof original==='function')original();renderEvidence()};window.V7ReleaseEnhancements={getSettings:()=>({...settings}),refreshEvidence:renderEvidence,refreshSettings:renderSettings};boot()}
