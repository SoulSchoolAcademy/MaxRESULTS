...
(() => {
  ...
  function remember() {
    return localStorage.getItem("nayanetName") || localState().name || "";
  }

  function slug(name) {
    return (name || "friend").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "friend";
  }
  ...
  function notify() {
    window.dispatchEvent(new CustomEvent("naya:data-ready", { detail: { memberId, ready } }));
  }
  ...
  async function hydrate() {
    ...
    if (profile.data?.public_alias) {
      s.name = profile.data.public_alias;
      writeLocal("nayanetName", profile.data.public_alias);
    }
  ...
  async function sync() {
    ...
    if (s.challenge) {
      await client.from("nayanet_challenges").upsert({
        member_id: memberId,
        goal: s.challenge.goal || "",
        current_day: s.challenge.day || 1,
        completed_days: s.challenge.done || []
      }, { onConflict: "member_id" });
    }
    if (Array.isArray(s.spaces) && s.spaces.length) {
      const rows = s.spaces.map(x => ({
        id: x.id || uuid(),
  ...
  Storage.prototype.setItem = function(key, value) {
    ...
    if (key === LOCAL_KEY && ready && !internalWrite) queueMicrotask(() => sync());
  };
