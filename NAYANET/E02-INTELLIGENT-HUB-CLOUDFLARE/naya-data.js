...
(() => {
  ...
  async function boot(){
    try{
      ...
      const name=remember();
      if(name){await client.from("members").upsert({id:memberId,display_name:name},{onConflict:"id"});await client.from("nayanet_profiles").upsert({member_id:memberId,smart_id:`naya/${slug(name)}`,public_alias:name},{onConflict:"member_id"})}
      ready=true;await hydrate();notify();
    }catch(error){ready=false;console.warn("NayaNET persistence bridge unavailable; local continuity remains active.",error);notify()}
  }
  ...
  window.addEventListener("naya:data-save",()=>sync());boot();
})();
