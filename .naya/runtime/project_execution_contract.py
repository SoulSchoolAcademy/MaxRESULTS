...
PROJECT=MEMORY/'projects'/'CURRENT-DAILY-PROJECT.json'
POLICY=MEMORY/'CONTINUITY-ENFORCEMENT-POLICY.json'
TORCH_CONTRACT=ROOT/'.naya'/'contracts'/'NEXT-ACTION-DELIVERY-CONTRACT-v1.json'
REPORT=MEMORY/'PROJECT-EXECUTION-CONTRACT-REPORT.json'
...
def _torch_required_fields(policy: dict[str,Any], contract: dict[str,Any]|None=None)->tuple[str,...]:
    if contract and contract.get('required_fields'): return tuple(contract['required_fields'])
    return tuple(policy.get('next_action_delivery_schema',{}).get('required_fields',('next_action','next_actor','ready_to_run_execution','expected_output','success_criteria','verification')))
...
def validate_next_action_delivery(n: dict[str,Any], policy: dict[str,Any]|None=None, *, human_continuation_required: bool|None=None) -> list[str]:
    ...
    required=_torch_required_fields(policy,contract)
    errors += [f'next action delivery missing {field}' for field in required if not _nonempty(n.get(field))]
    ...
    elif n.get('human_prompt_authoring_required') is not None and not isinstance(n.get('human_prompt_authoring_required'),bool):
        errors.append('human_prompt_authoring_required must be boolean when present')
    schema=(contract or {}).get('quality_rules',{}); minimum=int(schema.get('next_action_min_chars',24)); rejected={str(x).strip().lower() for x in schema.get('rejected_next_actions',[])}
    action=_text(n.get('next_action'))
    if action and (len(action)<minimum or action.lower() in rejected): errors.append('next_action is vague/non-executable')
    context_fields=tuple((contract or {}).get('quality_rules',{}).get('successor_context_fields',()) or policy.get('next_action_delivery_schema',{}).get('successor_context_fields',('mission','current_state','constraints','why_this_action')))
    ...
    if _nonempty(n.get('ready_to_run_execution')) and any(not _nonempty(context_values.get(field)) for field in context_fields): errors.append('successor cannot act without reconstructing context: ready_to_run_execution lacks required mission/state/constraints/why-this-action context')
    return errors
...
def validate():
    ...
    report={'schema_version':3,'status':'GREEN' if not errors else 'RED','meaningful_events_checked':checked,'error_count':len(errors),'errors':errors,'checks':['current_daily_project','project_event_binding','paired_representation_identity','next_execution_contract','next_action_delivery','human_continuation_authoring','successor_survival','learning_capture']}
...
def self_test():
    ...
    # Missing every required torch field must fail closed.
