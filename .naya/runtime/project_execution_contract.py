...
def self_test():
    ...
    bad=json.loads(json.dumps(base)); bad['project_context']['project_id']='WRONG'; bad['representations']['naya']['canonical_event_id']='WRONG'; bad_errors=validate_event(bad,project,policy); assert any('meaningful execution must bind' in x for x in bad_errors) and any('Naya representation is not bound' in x for x in bad_errors)
