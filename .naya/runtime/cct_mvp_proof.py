...
def independent_consume(serialized: str) -> dict:
    # Simulates a fresh Naya process receiving only the machine artifact.
...
def build_proof() -> dict:
    a = create_block(
        agent_id="naya-a",
        subject="maximum verified value per action",
        claim="A scarce execution resource should be spent only when it produces distinct verified value.",
        evidence=[{"type": "local_execution", "reference": "cct-mvp-proof"}],
        verification_method="deterministic CCTB protocol test",
        audience=["naya-b"],
        created_at="2026-08-29T00:00:00Z",
    )
    a_serialized = json.dumps(a, sort_keys=True, separators=(",", ":"))
    consumed = independent_consume(a_serialized)
    b = create_block(
        agent_id="naya-b",
        subject=consumed["subject"],
        claim="Independent consumers can verify permitted intelligence and create a traceable successor.",
        evidence=[{"type": "independent_consumption", "parent_block_id": consumed["block_id"]}],
        verification_method="independent Naya B consumption and successor test",
        audience=["naya-a"],
        parent={"block_id": consumed["block_id"], "block_hash": block_hash(a)},
    ...
    orphan = copy.deepcopy(a); orphan["producer"] = {"agent_id": "naya-c"}; orphan["claim"] = "THIS IS NOT EXECUTABLE"; orphan["block_id"] = block_hash(orphan)
    orphan["lineage"] = {"parent_block_id": "missing-parent", "parent_block_hash": "sha256:missing"}; orphan["block_id"] = block_hash(orphan)
    ...
    matrix = {
        ...
        "NEGATIVE TESTS": "GREEN" if all([
            expect_red("incomplete", validate_block(incomplete)),
            expect_red("malformed", validate_block(malformed)),
            expect_red("conversation", validate_block(conversation)),
            expect_red("orphan", validate_block(orphan)),
        ]) else "RED",
    ...
    if any(v != "GREEN" for v in matrix.values()):
        raise AssertionError(matrix)

    return {
        "protocol": "CCTB v0.1",
        "proof_version": "0.1",
        "source_context": "independent_artifact",
        "blocks": {"a": a, "b": b},
        "lineage": {"parent_block_id": b["lineage"]["parent_block_id"], "parent_block_hash": b["lineage"]["parent_block_hash"]},
        "matrix": matrix,
        "negative_tests": {
            "incomplete": expect_red("incomplete", validate_block(incomplete)),
            "malformed": expect_red("malformed", validate_block(malformed)),
            "conversation_dependent": expect_red("conversation", validate_block(conversation)),
            "invalid_orphan": expect_red("orphan", validate_block(orphan)),
            "tampered": expect_red("tampered", validate_block(tampered)),
            "unauthorized": expect_red("unauthorized", unauthorized),
            "bad_lineage": expect_red("bad_lineage", verify_link(bad_lineage, a)),
        },
        "first_pass": "GREEN",
    }
