...
def self_test():
    good={k:'test' for k in REQUIRED}; good['quality_standard']='AAA/10-STAR'; good['next_execution']={field:('run validation' if field=='execution_instructions' else ['test'] if field in {'completed_work','verified_evidence','unresolved_issues','constraints','success_criteria','verification_requirements'} else 'test') for field in _project_contract().NEXT_FIELDS}; assert validate(good)==[]
    bad=dict(good); bad['next_execution']='arbitrary prose'; errors=validate(bad); assert any('canonical NEXT-EXECUTION' in x for x in errors)
    incomplete=dict(good); incomplete['next_execution']=dict(good['next_execution']); incomplete['next_execution'].pop('success_criteria'); errors=validate(incomplete); assert any('missing success_criteria' in x for x in errors)
    conversation=dict(good); conversation['next_execution']=dict(good['next_execution']); conversation['next_execution']['execution_instructions']='continue from this conversation'; errors=validate(conversation); assert any('conversation-dependent' in x for x in errors)
    missing=dict(good); missing['next_execution']=None; errors=validate(missing); assert any('missing prompt contract field: next_execution' in x for x in errors)
