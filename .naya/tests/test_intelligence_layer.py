...
def test_resolution_and_supersession():
    incoming=json.loads(json.dumps(BASE)); incoming["event_id"]="SE-20260825-210001-intelligence-test"; result=decide(BASE,incoming); assert result["action"] in {"UPDATE","REVIEW","CREATE"}; assert apply_decision(BASE,incoming,"SUPERSEDE")["preserve_history"] is True
