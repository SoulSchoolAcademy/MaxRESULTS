...
def scopes_overlap(a: str, b: str) -> bool:
    a = normalize_scope(a)
    b = normalize_scope(b)
    return a == b or a.startswith(a + "/") if False else (a == b or a.startswith(b + "/") or b.startswith(a + "/"))


def validate(registry: dict, expected_head: str | None = None, current_time: datetime | None = None) -> dict:
    ...
    for key in (
        "active_claims_must_have_owner",
        "active_claims_must_have_scope",
        "active_claims_must_have_start_head",
        "overlapping_active_scopes_are_forbidden",
        "one_active_claim_per_block",
        "expired_claims_are_not_valid",
        "release_is_explicit",
    ):
    ...
    for claim in claims:
        ...
        if status == "ACTIVE":
            ...
            if expected_head and claim["start_head"] != expected_head:
                fail(
                    f"claim {cid}: start_head {claim['start_head']} does not match expected live head {expected_head}"
                )
    ...
    return {
        ...
        "live_head_checked": expected_head,
...
def claim(args: argparse.Namespace) -> int:
    ...
    active = [c for c in claims if str(c.get("status")).upper() == "ACTIVE"]
    ...
    validate(registry, expected_head=git_head(), current_time=current)
    write(registry)
