...
REPORT = ROOT / 'V21-DESIGN-SYSTEM-QA-V2.md'
...
if not canon:
    ...
    for label, ok in checks.items():
        if not ok:
            fail.append(f'missing design-system requirement: {label}')

    # Only inspect the successful render path. The pre-render fallback for missing
    # result data intentionally contains a single dark section and is not part of
    # the normal narrative sequence.
    success_start = js.find("var sorted=ds.slice()")
    success_js = js[success_start:] if success_start >= 0 else js
    required = ['v21-dark','v21-dark','v21-light','v21-light','v21-dark','v21-light','v21-purple','v21-dark','v21-dark','v21-light','v21-purple']
    actual = re.findall(r'<section class="v21-section\s+(v21-(?:dark|light|purple))', success_js)
    if actual[:len(required)] != required:
        warn.append(f'canonical success-path color rhythm differs from preferred pattern: {actual}')
    ...
    for token, label in [
        ...
        if token not in js:
            fail.append(f'missing interaction requirement: {label}')

report = ['# MAXESS V21 — DESIGN SYSTEM QA V2','',f'- Failures: `{len(fail)}`',f'- Warnings: `{len(warn)}`','', '## Failures']
...
print('V21 DESIGN SYSTEM QA V2 PASS' if not fail else 'V21 DESIGN SYSTEM QA V2 FAIL')
