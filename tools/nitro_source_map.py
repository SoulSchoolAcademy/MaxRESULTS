...
def main():
    ...
    for i, m in enumerate(scripts, 1):
        ...
        out.append(f"- Script {i}: source line {line_no(text, m.start())}; end line {line_no(text, m.end())}; chars {len(body)}; first code: `{first}`")
    out += ["", "## Inline styles"]
    for i, m in enumerate(styles, 1):
        body = m.group(1)
        first = next((ln.strip() for ln in body.splitlines() if ln.strip()), "")[:140]
        out.append(f"- Style {i}: source line {line_no(text, m.start())}; end line {line_no(text, m.end())}; chars {len(body)}; first rule: `{first}`")
