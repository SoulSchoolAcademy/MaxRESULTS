# NAYA NOTES / SMART NOTES — RETRIEVAL INDEX

**Synonyms:** Naya Note = Smart Note = durable Naya memory.

## Purpose

This index is the human/AI navigation layer for durable project memory.

It exists because an AI should be able to find relevant knowledge even when it does **not** know the exact wording used in the original conversation or note title.

The index is not a second authority. The individual Smart Notes and the governing documents remain the evidence; this file helps locate them.

## Retrieval strategy

When searching memory, use a **concept cluster**, not one exact keyword.

Search in this order:

1. natural-language concept;
2. likely synonyms/aliases;
3. category;
4. date/time range;
5. project/feature;
6. related document/path;
7. note content/evidence.

Example:

A request such as **“What did we learn about making Naya remember things?”** should search concepts including:

`Naya Note · Smart Note · memory · durable memory · project brain · recall · context restoration · learning · notes`

Do not require the user or AI to know the exact phrase **“Smart Notes System.”**

## Directory convention

Notes are stored by creation year/month:

`docs/smart-notes/YYYY/MM/YYYY-MM-DD-short-topic.md`

The filename is a locator, not the authority.

## Primary memory taxonomy

- **IDEA** — concept or possibility
- **LEARNING** — discovery
- **GOAL** — desired outcome
- **DECISION** — explicit decision
- **FACT** — important fact
- **RESOURCE** — useful reference
- **KNOWLEDGE** — durable product/domain knowledge
- **PROBLEM** — failure or issue
- **SOLUTION** — solution/guardrail
- **PERSON** — useful person information
- **EVENT** — event/milestone
- **TASK** — action
- **INSIGHT** — meaningful realization/pattern

## Current note inventory

### 2026-08-18

| Topic | Primary category | Useful retrieval terms | Note |
|---|---|---|---|
| Naya Nitro North Star / executive execution plan | DECISION | Naya Nitro, Smart Notes, Naya Notes, North Star, Take the Lead, AAA, MaxIS, master execution, next action, project brain | `2026-08-18-naya-nitro-north-star.md` |
| Cold-start governance repair / main-active branch model | SOLUTION | cold start, governance, main branch, active branch, repository map, branch divergence, source of truth, governance bridge, AI comprehension, Section 01 | `2026-08-18-cold-start-governance-repair.md` |

## Cross-system retrieval map

| If the user asks about… | Start with… | Then inspect… |
|---|---|---|
| Naya's overall operating philosophy | `NAYA-OS.md` | Executive Plan, Blueprint, relevant notes |
| What Naya Nitro is | `docs/NAYA-NITRO-MASTER-BLUEPRINT.md` | Executive Plan, relevant notes |
| North Star / what-why-how | `docs/NAYA-EXECUTIVE-PLAN.md` | North Star Smart Note |
| How to execute work | `docs/NAYA-NITRO-MODE.md` | Execution Prompt Template, learning log |
| Reusable execution prompt | `docs/NAYA-EXECUTION-PROMPT-TEMPLATE.md` | current task-specific notes |
| Naya Note / Smart Note / memory | `docs/NAYA-SMART-NOTES-SYSTEM.md` | this index, relevant notes |
| What was learned | relevant dated Smart Notes | Learning Log |
| What changed / was decided | DECISION notes by date/topic | Executive Plan / product spec if promoted |
| Cold-start / branch governance | `2026-08-18-cold-start-governance-repair.md` | `main:START-HERE.md`, active branch START-HERE |
| MAXESS Results | `docs/MAXESS-RESULTS-PRODUCT-SPEC.md` | Operating Manual, task-specific notes |
| Source of truth / current state | `docs/REPOSITORY-MAP.md` | `docs/SOURCE-AND-MEMORY-MAP.md` |
| Live/deployment status | `docs/DEPLOYMENT-CONTRACT.md` | Release Checklist |

## Update rule

When a new durable note is created or an existing durable subject materially changes:

1. follow `docs/NAYA-SMART-NOTES-SYSTEM.md`;
2. add/update the note;
3. update this index when the note is a meaningful new retrieval target or changes a major topic;
4. cross-link related notes/documents;
5. promote true governing rules into the correct authority document;
6. verify the saved file and index before claiming completion.

Do not create duplicate notes merely because the user used a new synonym.

## Searchability law

Every durable note must include:

- descriptive title;
- timestamp;
- one primary category;
- 5–15 plain-language keywords;
- aliases/synonyms;
- related paths/concepts;
- context;
- durable lesson/decision;
- why it matters;
- required behavior;
- evidence/source.

This makes the memory system discoverable by meaning rather than exact wording.
