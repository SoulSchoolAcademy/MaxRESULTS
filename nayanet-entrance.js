(() => {
  ...
  /* NayaNET — Legendary Threshold Controller
     Identity is entered fresh on every entrance.
     Activation is owned by the Intelligent Vault controller loaded after this file.
     The legacy controller must never prefill a prior name or race the vault transition.
  */
  ...
  /* Never prefill the identity field. The saved identity remains available to the app
     after entry, but the front door always asks the arriving human who they are. */
  ...
  /* The vault controller owns submit in capture phase. This fallback remains for
     accessibility/API callers and cannot race the vault because the vault handler
     stops propagation before this bubble-phase listener can run. */
  form.addEventListener('submit', event => {
    event.preventDefault();
    const name = clean(input.value);
    if (!name) { input.focus(); return; }
    saveIdentity(name);
    window.NayaNETVault?.activate?.();
  });
  ...
  window.NayaNETThreshold = Object.freeze({
    ...
    enter: () => window.NayaNETVault?.activate?.(),
    refresh: sync
  });
})();
